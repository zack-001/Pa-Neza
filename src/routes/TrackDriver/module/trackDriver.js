...
export function getDriverInfo(){
	return (dispatch, store)=>{
		let id = store().HomeReducer.booking.driverId;
		request.get("http://10.0.2.2:4000/api/driver/" + id)
		.finish((error, res)=>{
			dispatch({
...
export function getDriverLocation(){
	return (dispatch, store)=>{
		let id = store().HomeReducer.booking.driverId;
		request.get("http://10.0.2.2:4000/api/driverLocation/" + id)
		.finish((error, res)=>{
			dispatch({
...
export function getDistanceFromDriver(){
	return (dispatch, store)=>{
		if(store().trackDriver.driverLocation){
			...
			.query({
				origins:store().HomeReducer.selectedAddress.selectedPickUp.location.latitude + 
				"," + store().HomeReducer.selectedAddress.selectedPickUp.location.longitude,
